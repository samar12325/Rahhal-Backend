...
var AiTripsService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.AiTripsService = void 0;
...
const AI_COPY = {
    ar: {
        ...
        assistantOverview: 'استخدم هذا المسار للدردشة الحرة عن المدن والأجواء والمقارنات قبل ما تقرر جدولك.',
        styles: {
            all: 'متوازن',
            family: 'عائلية',
            adventure: 'مغامرات',
        ...
        sessionTitle: (city, days) => `رحلة ${days} أيام في ${city}`,
        styleTitle: (style) => `إيقاع ${style}`,
        daySubtitle: (city, label) => `${label} داخل ${city}`,
        planTitle: (city, days) => `خطة ${days} أيام في ${city}`,
        summaryOverview: (city, style, audience) => `خطة منظمة لزيارة ${city} بروح ${style} ومناسبة لـ ${audience}.`,
        ...
        budgetLabel: (minBudget, maxBudget) => `${minBudget} - ${maxBudget}`,
        plannerTips: {
            budget: 'احجز الأنشطة المدفوعة مبكراً وخلك مرن في الوجبات لتوازن الميزانية.',
        ...
        assistantReplies: {
            ...
            askClarify: 'قل لي: تبي بحر، جو بارد، أماكن تصوير، مطاعم، أو رحلة عائلية؟ وأنا أضبط الترشيح لك.',
            noExactMatch: 'أقدر أساعدك، لكن اذكر لي اسم مدينة أو نوع التجربة التي تبحث عنها حتى يكون الجواب أدق.',
            followUp: 'إذا تبغى، أقدر بعد ذلك أحول الكلام هذا إلى جدول يومي منظم بضغطة واحدة.',
        },
    },
    en: {
        ...
        assistantOverview: 'Use this mode for open-ended travel chat about cities, atmosphere, and comparisons before generating an itinerary.',
        styles: {
            all: 'Balanced',
            family: 'Family',
            adventure: 'Adventure',
        ...
        sessionTitle: (city, days) => `${days}-day trip in ${city}`,
        styleTitle: (style) => `${style} pace`,
        daySubtitle: (city, label) => `${label} in ${city}`,
        planTitle: (city, days) => `${days}-day plan for ${city}`,
        summaryOverview: (city, style, audience) => `A structured ${style.toLowerCase()} itinerary for ${city}, suitable for ${audience}.`,
        audienceLabel: (people) => {
            if (people <= 1)
                return 'solo travel';
            if (people === 2)
                return 'two travelers';
            if (people <= 5)
                return `${people} travelers`;
            return `a group of ${people}`;
        },
        budgetLabel: (minBudget, maxBudget) => `${minBudget} - ${maxBudget}`,
        plannerTips: {
            budget: 'Book major paid activities early and keep meal choices flexible to balance budget.',
            pace: 'Spread demanding activities across different days to keep the trip comfortable.',
            family: 'Use shorter stops if you are traveling with children or older family members.',
            photo: 'Keep one golden-hour slot before sunset for photos and a light walk.',
        },
        plannerReplies: {
            default: 'I refined the itinerary and made it more balanced.',
            cheaper: 'I reduced the estimated cost and leaned toward budget-friendly options.',
            nature: 'I added nature and scenic stops to suitable days.',
            restaurants: 'I added clearer food and dinner suggestions across the itinerary.',
            shopping: 'I inserted shopping time without squeezing the rest of the day.',
            family: 'I shifted the itinerary to a more family-friendly pace.',
            day2: 'I made day two lighter than the rest of the itinerary.',
        },
        assistantReplies: {
            compareIntro: 'If you are choosing between the two, here is the practical difference:',
            recommendationIntro: 'These are good starting recommendations:',
            askClarify: 'Tell me if you want sea, cool weather, photo spots, restaurants, or a family trip and I will narrow it down.',
            noExactMatch: 'I can help, but mention a city name or the type of experience you want so the answer can be more specific.',
            followUp: 'If you want, I can turn this into a structured day-by-day itinerary next.',
        },
    },
};
let AiTripsService = AiTripsService_1 = class AiTripsService {
    prisma;
    config;
    logger = new common_1.Logger(AiTripsService_1.name);
    ...
    async getSession(userId, sessionId) {
        ...
        if (!session) {
            throw new common_1.NotFoundException('AI trip session not found');
        }
    ...
    async generate(userId, dto) {
        ...
        const created = await this.prisma.ai_trip_sessions.create({
            data: {
                ...
                days: plan.days,
                people: dto.people,
                min_budget: dto.minBudget !== undefined
                    ? new client_1.Prisma.Decimal(dto.minBudget)
                    : null,
                max_budget: dto.maxBudget !== undefined
                    ? new client_1.Prisma.Decimal(dto.maxBudget)
                    : null,
    ...
    async startAssistantSession(userId, dto) {
        const locale = this.resolveLocale(dto.locale);
        const message = dto.message.trim();
        const assistant = await this.generateAssistantReply(message, locale);
        const now = new Date();
        const assistantContext = assistant.contextLabel ||
            assistant.primaryDestination?.name ||
            this.extractAssistantContextLabel(message, locale);
        const plan = this.buildAssistantPlaceholderPlan(locale, assistantContext || AI_COPY[locale].unknownCity);
        const created = await this.prisma.$transaction(async (tx) => {
            ...
            await tx.ai_trip_messages.create({
                data: {
                    ...
                    role: client_1.ai_trip_message_role.user,
                    text: message,
                },
            });
            await tx.ai_trip_messages.create({
                data: {
                    session_id: session.id,
                    role: client_1.ai_trip_message_role.assistant,
    ...
    async chat(userId, sessionId, dto) {
        ...
        const isAssistantSession = this.getSessionMode(session.style) === 'assistant';
        const now = new Date();
        let reply = '';
        ...
        if (isAssistantSession) {
            const assistant = await this.generateAssistantReply(dto.message.trim(), locale, session.ai_trip_messages);
            const assistantContext = assistant.contextLabel ||
                assistant.primaryDestination?.name ||
            ...
            const updated = await this.patchPlannerPlan(dto.message.trim(), currentPlan, locale, session.ai_trip_messages);
            reply = updated.reply;
    ...
    getSessionMode(style) {
        return style === SESSION_STYLE_ASSISTANT ? 'assistant' : 'planner';
    }
    buildAssistantSessionTitle(message) {
    ...
    getOpenAiModel() {
        return this.config.get('OPENAI_MODEL')?.trim() || 'gpt-4.1-mini';
    }
    async buildPlannerPlan(dto, locale) {
    ...
    async buildPlannerPlanFallback(dto, locale) {
        ...
        const minBudget = dto.minBudget !== undefined && dto.minBudget !== null ? dto.minBudget : 0;
        const maxBudget = dto.maxBudget !== undefined && dto.maxBudget !== null
            ? dto.maxBudget
            : '∞';
        const style = dto.style?.trim() || 'all';
        const styleLabel = copy.styles[style] ?? copy.styles.all;
    ...
    buildAssistantPlaceholderPlan(locale, city) {
        const copy = AI_COPY[locale];
        return {
            city,
            days: 1,
    ...
    buildPlannerDayItems(focus, locale, people) {
        const copy = AI_COPY[locale];
    ...
    buildHighlights(style, prefs, locale, destination) {
        const copy = AI_COPY[locale];
        const items = [];
        if (destination?.region) {
            items.push(copy.regionLabels[destination.region] ?? destination.region);
        }
        const styleLabel = copy.styles[style];
        if (styleLabel) {
            items.push(styleLabel);
        }
        prefs.forEach((pref) => {
            const label = copy.prefs[pref];
            if (label)
                items.push(label);
        });
        return [...new Set(items)].slice(0, 5);
    }
    buildTips(style, prefs, locale) {
        const copy = AI_COPY[locale];
        const tips = [copy.plannerTips.budget, copy.plannerTips.pace];
        if (style === 'family') {
    ...
    recalculatePlan(plan) {
        const daysPlan = plan.daysPlan.map((day) => {
            const estimatedTotal = day.items.reduce((sum, item) => sum + Number(item.estimatedCost || 0), 0);
            return {
                ...day,
                estimatedTotal,
            };
        ...
        return {
            ...plan,
            daysPlan,
            estimatedTotal: daysPlan.reduce((sum, day) => sum + Number(day.estimatedTotal || 0), 0),
        };
    }
    async enhancePlannerPlanWithOpenAi(dto, locale, draftPlan) {
        const copy = AI_COPY[locale];
        const payload = await this.requestOpenAiStructuredResponse({
        ...
        const aiPlan = this.readPlan(payload);
        return this.mergePlanWithFallback(aiPlan, draftPlan);
    }
    async patchPlannerPlanWithOpenAi(message, plan, locale, history) {
        const copy = AI_COPY[locale];
        ...
        const nextPlan = this.mergePlanWithFallback(this.readPlan(payload.plan), plan);
        return {
            reply: payload.reply?.trim() || copy.plannerReplies.default,
            patchedPlan: nextPlan,
        };
    }
    async generateAssistantReplyWithOpenAi(message, locale, history) {
        const copy = AI_COPY[locale];
        ...
        return {
            ...
            primaryDestination: this.findDestinationByName(destinations, payload.primaryDestinationName),
    ...
    mergePlanWithFallback(aiPlan, fallbackPlan) {
        const daysPlan = fallbackPlan.daysPlan.map((fallbackDay, index) => {
            const aiDay = aiPlan.daysPlan[index];
            const nextItems = Array.isArray(aiDay?.items) && aiDay.items.length > 0
                ? aiDay.items
                : fallbackDay.items;
            return {
                ...fallbackDay,
        ...
        return this.recalculatePlan({
            ...
            highlights: this.pickBestStringList(aiPlan.highlights, fallbackPlan.highlights, 5),
            tips: this.pickBestStringList(aiPlan.tips, fallbackPlan.tips, 4),
            daysPlan,
        });
    }
    pickBestStringList(primary, fallback, limit) {
    ...
    mapHistoryToOpenAiInput(history) {
        return history
            .filter((item) => item?.text?.trim())
            .map((item) => ({
            role: item.role === client_1.ai_trip_message_role.assistant ? 'assistant' : 'user',
            content: item.text.trim(),
        }));
    }
    formatDestinationCatalog(destinations, locale) {
        const copy = AI_COPY[locale];
        return destinations
            .map((destination) => {
            const region = copy.regionLabels[destination.region] ?? destination.region;
    ...
    findDestinationByName(destinations, name) {
    ...
    async requestOpenAiStructuredResponse({ instructions, input, schemaName, schema, maxOutputTokens, temperature, }) {
        ...
        if (!response.ok) {
            throw new Error(this.readOpenAiErrorMessage(payload) ||
    ...
    logAiServiceFallback(context, error) {
    ...
    async patchPlannerPlan(message, plan, locale, history = []) {
        if (!this.hasOpenAiApiKey()) {
            return this.patchPlannerPlanFallback(message, plan, locale);
        }
        try {
            return await this.patchPlannerPlanWithOpenAi(message, plan, locale, history);
        }
        catch (error) {
    ...
    patchPlannerPlanFallback(message, plan, locale) {
        const copy = AI_COPY[locale];
        ...
        let reply = copy.plannerReplies.default;
        if (hasAny([
            'أرخص',
            'ارخص',
            'قلل',
        ...
        if (hasAny(['طبيعة', 'nature', 'outdoor', 'scenic', 'views'])) {
            addItemToEachDay({
                ...
                name: locale === 'ar' ? 'توقف طبيعي / نقطة مطل' : 'Nature stop / viewpoint',
                estimatedCost: 0,
                note: locale === 'ar'
        ...
        if (hasAny(['مطاعم', 'أكل', 'اكل', 'food', 'restaurant', 'restaurants'])) {
            addItemToEachDay({
                time: '20:00',
                tag: copy.timeTags.night,
                name: locale === 'ar' ? 'اقتراح عشاء واضح' : 'Clear dinner recommendation',
                estimatedCost: 90,
                note: locale === 'ar'
    ...
    async generateAssistantReply(message, locale, history = []) {
        if (!this.hasOpenAiApiKey()) {
            return this.generateAssistantReplyFallback(message, locale);
        }
        try {
            return await this.generateAssistantReplyWithOpenAi(message, locale, history);
        }
        catch (error) {
    ...
    async generateAssistantReplyFallback(message, locale) {
        const copy = AI_COPY[locale];
        ...
        const matched = destinations.filter((destination) => normalized.includes(this.normalizeText(destination.name)));
        ...
        if (wantsRecommendations) {
            ...
            const reply = [
                ...
                '',
                ...suggestions.map((destination, index) => `${index + 1}. ${destination.name}: ${this.describeDestination(destination, locale, { wantsFamily, wantsBudget })}`),
                '',
                copy.assistantReplies.askClarify,
            ].join('\n');
            return {
    ...
    describeDestination(destination, locale, options) {
        const copy = AI_COPY[locale];
        const region = copy.regionLabels[destination.region] ??
            destination.region;
    ...
    suggestTripLength(destination, scenicBias, locale) {
    ...
    pickDestinationSuggestions(destinations, hints) {
        const filtered = destinations.filter((destination) => {
            const haystack = this.normalizeText(`${destination.name} ${destination.description ?? ''}`);
            if (hints.wantsSea &&
                !this.containsAny(haystack, ['بحر', 'sea', 'coast', 'كورنيش'])) {
                return false;
            }
            if (hints.wantsNature &&
    ...
    extractAssistantContextLabel(message, locale) {
        const trimmed = message.replace(/\s+/g, ' ').trim();
        if (!trimmed) {
            return null;
        }
        const topicPatterns = locale === 'ar'
            ? [
                /(مطاعم\s+[^\s?.!,،]+)/,
                /(مقاهي\s+[^\s?.!,،]+)/,
                /(كافيهات\s+[^\s?.!,،]+)/,
        ...
        const locationMatch = locale === 'ar'
            ? trimmed.match(/(?:في|ب|عن|داخل|حول|قرب)\s+([ء-يA-Za-z][ء-يA-Za-z\s-]{1,30})/u)
            : trimmed.match(/(?:in|about|around|near)\s+([a-zA-Z][a-zA-Z\s-]{1,30})/i);
        if (locationMatch?.[1]) {
    ...
    cleanAssistantContextLabel(value) {
        return value
            .replace(/[?.!,،]/g, ' ')
            .replace(/\s+/g, ' ')
            .trim()
    ...
    async findDestinationByCity(city) {
        if (!city ||
    ...
    readPlan(value) {
        const fallback = this.buildAssistantPlaceholderPlan('ar', AI_COPY.ar.unknownCity);
        ...
        const parsedDaysPlan = daysPlanValue
            ...
            return {
                ...
                title: typeof day.title === 'string'
                    ? day.title
                    : index === 0
                        ? fallback.tripTitle
                        : `Day ${index + 1}`,
                subtitle: typeof day.subtitle === 'string' ? day.subtitle : undefined,
                estimatedTotal: typeof day.estimatedTotal === 'number' ? day.estimatedTotal : 0,
                items: parsedItems,
            };
        })
            .filter(Boolean);
        return this.recalculatePlan({
            city: typeof plan.city === 'string' && plan.city.trim()
                ? plan.city
                : fallback.city,
            days: typeof plan.days === 'number' && plan.days > 0
                ? plan.days
                : fallback.days,
            budgetLabel: typeof plan.budgetLabel === 'string'
                ? plan.budgetLabel
                : fallback.budgetLabel,
            tripTitle: typeof plan.tripTitle === 'string'
                ? plan.tripTitle
                : fallback.tripTitle,
            overview: typeof plan.overview === 'string' ? plan.overview : fallback.overview,
            estimatedTotal: typeof plan.estimatedTotal === 'number'
                ? plan.estimatedTotal
                : fallback.estimatedTotal,
            styleLabel: typeof plan.styleLabel === 'string'
                ? plan.styleLabel
                : fallback.styleLabel,
            audienceLabel: typeof plan.audienceLabel === 'string'
                ? plan.audienceLabel
                : fallback.audienceLabel,
            highlights: Array.isArray(plan.highlights)
                ? plan.highlights.filter((item) => typeof item === 'string')
                : [],
            tips: Array.isArray(plan.tips)
                ? plan.tips.filter((item) => typeof item === 'string')
                : [],
...
exports.AiTripsService = AiTripsService;
exports.AiTripsService = AiTripsService = AiTripsService_1 = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_service_1.PrismaService,
        config_1.ConfigService])
], AiTripsService);
//# sourceMappingURL=ai-trips.service.js.map